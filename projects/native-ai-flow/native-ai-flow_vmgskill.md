...
description: 借鉴原生 AI 工作流的玻璃层次与扫光动作，或改作完整架构动画；说明跨用途复用及模块、连线、材质和时间轴的依赖。
metadata:
...
用于借鉴本工程的视觉与动态做法，或把用户提供的系统架构、业务流程和模块关系做成可编辑动画。沿用用户的内容、风格和运动要求；模块数量、布局、配色和时长都可以调整。
...
玻璃透射背景、分区与卡片的材质层次、边缘高光及扫光接力可以用于产品卡片、标题或其他信息画面。模型按新内容决定元素数量、布局和时序，架构用途仅提供模块与连线的组织例子。

- 只借鉴观感或动作方式时，可以在新工程中设计自己的组件和轨道，无需保留原有 SDK、CLI 等模块。
- 复用玻璃实现时，核对 `src/effects.ts` 的定义注册、`src/recovered/implementation.js` 的效果回调、背景采样及目标节点参数。只复制 `liquid-glass` 实例数据不会带来效果实现；采样结果还依赖背景与图层顺序。
- 局部迁移卡片或动作时，重建目标节点的父层级和轨道引用。`standardSchema`、`nativeFlowProOwners`、`pathFollow` 绑定的是原工程节点，按新场景更新实际需要的关联，不让这些旧绑定决定新作品的结构。

下面的字段与改作步骤对应原工程；只借鉴局部时按需阅读相关部分。
