...
description: 借鉴 Duo 展开 2D 的屏幕光感、玻璃元素与分段运动，或改作双屏展示；说明跨用途参考和设备几何、内容、投影的依赖。
metadata:
...
用于借鉴本工程的屏幕光感与运动做法，或制作壁纸、应用界面和双屏设备展示。展示内容、配色、构图与节奏按用户需求调整，不要求作品归入某个风格。
...
可以参考屏幕反光、玻璃文字与按钮的观感，以及形态变化、壁纸缩放和内容显现之间的节奏关系。即使新作品不是手机展示，也可由模型根据新内容重新组织这些做法；用途标签只辅助理解设备与内外屏内容的归属。

- 观感和时序可作为设计参考，目标场景的元素、布局与动作由新需求决定，不需要保留手机或双屏结构。
- 复用现有实现时，屏幕投影、反光与快捷按钮涉及 `controllerId`、`screenId`、`surface` 和设备几何；外框与遮罩又依赖 `duo-phone-frames`。需要读取组件、效果声明及恢复回调，不能把一个屏幕节点当作独立通用玻璃组件移走。
- 若目标只是平面卡片或文字，可借鉴材质观感与分段进度，在目标组件中实现相应绘制；确需沿用设备投影时，保留或改造几何、素材、控制节点和合成关系。相关字体与图片仍按其许可使用。

下面的参数和轨道说明针对沿用本工程的改作；局部借鉴时只读取相关依赖。
