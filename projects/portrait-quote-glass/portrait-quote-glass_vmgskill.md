...
description: 借鉴人物金句模板的玻璃层次、线条展开与漂移动画，或改作完整人物画面；说明跨用途复用、自适应文字与材质实现的依赖。
metadata:
...
用于借鉴本模板的玻璃与展开风格，或把用户的人物图片、姓名、身份和金句放入可编辑工程。按当前用户要求组织布局、材质与运动；原有配色、尺寸、时长和动效均可调整。
...
从线条生长到玻璃卡片展开的动作、厚度与边缘光学表现、文字显现和轻微漂移可以用于流程节点、产品标签或其他信息卡片，不必先选择“人物介绍”用途。模型根据新内容组织节点、关系和时序；人物、署名与引文排布只作为需要时的辅助例子。

- 只参考动作与观感时，可以在新组件中重新实现线条、几何展开及后续显现。原有 6.3 秒时长、人物层级和文字顺序不约束新作品。
- 复用自适应卡片实现时，核对 `src/auto-card.tsx`、玻璃模块、`src/font-metrics.ts`、实际字体资源及入口注册。玻璃 Surface 在卡片内部创建，参数位于 `props.data`；它不是可直接拷到任意节点 `effects` 数组的实例。
- 保留“线条到玻璃展开”时，需要同时处理布局计算与 `drawProgress` / `expansion`；采样依赖背后的场景及节点顺序。迁移漂移时将相关轨道绑定到目标节点，区别位置组、入场组与持续运动，避免把原来的人物布局一并带入。

下面的字段描述用于完整改作或查阅相关实现；只参考局部时按需阅读。
