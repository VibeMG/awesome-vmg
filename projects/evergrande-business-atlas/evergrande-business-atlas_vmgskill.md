...
description: 借鉴企业业务版图的深色卡片、线条生长与镜头巡游，或改作完整架构；说明跨用途迁移与摄影机、卡片、连线合成的依赖。
metadata:
...
用于借鉴本案例的深色卡片与镜头风格，或替换企业内容、调整结构和镜头，继续交付可编辑 VMG 工程。原作采用 S02 深色商务＋S04 线性科技；3D 镜头巡游是运动方式，共面图文卡片构成 2.5D 信息图。用户可以改变内容、配色、布局、卡片数量和运动，不必保持八个平台或 15 秒。
...
深色渐变卡片、细线生长、照明变化和镜头由局部巡游到全景的表达，可以用于产品功能、知识关系或其他内容。模型按新主题组织画面与节奏，企业分支关系只提供需要时的布局参考，原有八个平台和历史内容不作为结构要求。

- 可以单独参考 `CardFace` 与 `src/scene.css` 中的卡片视觉，或借鉴镜头节奏并在新场景中安排自己的观看顺序；主体、分支和文案由用户内容决定。
- 复用整套空间实现时，要连同 `ArchitectureScene` 的摄影机绘制、卡片收集、连线投影和组件注册一起核对。原有卡片、连线、字幕组件自身 `render` 返回 `null`，只移植这些数据节点不会独立产生画面；若改成独立绘制组件，需要相应实现。
- 镜头与连线沿同一投影体系工作，卡片、端点引用和时间轴应对应新场景。只借用静态外观时可另做绘制；沿用空间表现时保留必要的图片准备、资源加载和字体声明，素材仍按原许可处理。

下面的节点与参数说明针对原工程，完整改作或局部迁移时按实际依赖查阅。
