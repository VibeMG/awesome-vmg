...
- [完整 VMG 工程](projects/README.md)：已收录2个恢复源码工程：液态玻璃架构演示与Duo展开2D特殊案例，均附 `_vmgskill.md` 生成与改作指南。
...
工程索引已收录2份作者提供的`.vmg`恢复源码包；打开方式、版本与验证范围见各工程说明，无需改造CLI。外部链接、视频预览和 `.vmgc` 编译预览不冒充完整源码工程。

## 来源与许可

本库自己编写的说明、索引与脚本采用 [Apache-2.0](LICENSE)。外部作品、源码、媒体、品牌与商标仍遵循各自的授权；链接公开可见不意味着素材可以随库再分发。

外部参考保存链接与借鉴说明；源码工程另附作者提供的原始包和包内静态预览，许可及恢复边界见对应条目。各条目的查看记录区分画面、预览、介绍页和检索线索，保留实际观察范围。
